...
public interface PagamentoRepository extends JpaRepository<Pagamento, Long> {
	...
	@Query(value = " Select id, tipo, data, fornecedor, moeda,  origem,  valor, anexo  from vw_pagamentos  where (id like %:search%  or  tipo like %:search% " +
	" or  data like %:search% or fornecedor like %:search% or moeda like %:search%  or origem like %:search%  or valor like %:search% ) and restrito='Nao' " , nativeQuery = true)
    Page<listaSaidasGerais> findByPagamentoRestrito(@Param("search") String search, Pageable pageable); 
