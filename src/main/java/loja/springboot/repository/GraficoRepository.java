		...
		List<listGraficoPrincipal> graficoPrincipal();
			...
			List<listGraficoCard> graficoCard();
				public static interface listGraficoCard {
					...
					String getTempo(); 
					String getProva(); 
					String getLocacaoTotal(); 
					String getQuantidadeCidades(); 
			...
			List<listGraficoOrigemCLiente> graficoOrigemCliente();
				...
				@Query(value = " Select  tabela from painel_locacao_atendente", nativeQuery = true)
					List<listGraficoLocacaoAtendente> graficoLocacaoAtendente();
						public static interface listGraficoLocacaoAtendente {
							String getTabela(); 							
						} 

				@Query(value = " Select Locacao, Oportunidades, Indice,  EventoFuturos, Cac, Indicador, PercentualAds, NegociosPerdidos from painel_indicador_secundario", nativeQuery = true)
