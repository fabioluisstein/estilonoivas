		...
		columns : [
				...
				{	data : 'id',	
					...
					render : function(id) {
						return ''.concat('<a class="btn btn-info btn-sm btn-block"', ' ') 
								 ...
								 .concat('role="btn-group btn-group-sm" title="Editar" data-toggle="tooltip" data-placement="right">', ' ')
				...
				"width": '5%',
				 render: function ( data, type, row) {	 	
				  return  '<td <i class="fas fa-edit"></i></a> <div> </div>  <button onclick="if (confirm(\'Deseja excluir?\')) { window.location.href = \'/u/removerusuario/' + row.id +'\'; }  else {  }" class="btn btn-danger"><i class="fas fa-trash"></i></button> </div> </td>';
			  }
		   }
		]
	});
